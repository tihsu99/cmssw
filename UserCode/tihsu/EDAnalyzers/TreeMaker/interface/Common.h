...
namespace Common
{
    ...
    // Will get the hardest obj1 matched to obj2 s.t. dR(obj1, obj2) < maxDeltaR
    // For e.g. obj1 can be reco-electrons and obj2 can be gen-electrons
    void getHardestInCone(
        std::vector <CLHEP::HepLorentzVector> v_obj1_4mom,
        std::vector <CLHEP::HepLorentzVector> v_obj2_4mom,
        std::vector <int> &v_obj1_matchedObj2_index, // Will be of v_obj1_4mom.size(). Will be filled with the index of the matched obj1; if not matched, then -1.
        std::vector <double> &v_obj1_matchedObj2_deltaR, // Will be of v_obj1_4mom.size(). Will be filled with dR(obj1, obj2) if matched; if not matched, then -1.
        double maxDeltaR,
        double defaultVal = Constants::_defaultVal_pos1
    )
    {
        ...
        v_obj1_matchedObj2_index.clear();
        v_obj1_matchedObj2_index.resize(nObj1, -1);
        
        v_obj1_matchedObj2_deltaR.clear();
        v_obj1_matchedObj2_deltaR.resize(nObj1, defaultVal);
        
        
        for(int iObj2 = 0; iObj2 < nObj2; iObj2++)
        {
            int obj1_idx = -1;
            double minDeltaR = Constants::_defaultVal_pos1;
            
            for(int iObj1 = 0; iObj1 < nObj1; iObj1++)
            {
                if(v_obj1_matchedObj2_index.at(iObj1) >= 0)
                {
                    continue;
                }
                ...
                //if((obj1_idx < 0 && deltaR < maxDeltaR) || (deltaR < minDeltaR && v_obj1_4mom.at(iObj1).perp() > v_obj1_4mom.at(obj1_idx).perp()))
                //{
                //    obj1_idx = iObj1;
                //    minDeltaR = deltaR;
                //}
                
                if(deltaR < maxDeltaR && (obj1_idx < 0 || v_obj1_4mom.at(iObj1).perp() > v_obj1_4mom.at(obj1_idx).perp()))
                {
                    obj1_idx = iObj1;
                    minDeltaR = deltaR;
                }
            ...
            if(obj1_idx >= 0)
            {
                v_obj1_matchedObj2_index.at(obj1_idx) = iObj2;
                v_obj1_matchedObj2_deltaR.at(obj1_idx) = minDeltaR;
            }
        }
    }
